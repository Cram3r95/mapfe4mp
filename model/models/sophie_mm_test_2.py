...
class MultiheadSelfAttention_CRAT(nn.Module):
    ...
    def forward(self, att_in, agents_per_sample):
        ...
        # Upper path is faster for multiple samples in the batch and viceversa
        ...
        if len(agents_per_sample) > 1 or test:
            ...
            padded_att_in_swapped = self.ln(padded_att_in_swapped)

            if torch.any(torch.isnan(padded_att_in_swapped)):
                pdb.set_trace()
...
#         self.concat_centerlines = CONCAT_CENTERLINES
...
#         aux_length = int(math.floor((self.lane_length*self.concat_centerlines-sub_)/pow(2,num_poolings)))
...
#         self.bn1 = nn.BatchNorm1d(self.lane_length*self.concat_centerlines)
...
#             # plausible_area = torch.clone(phy_info) # TODO: Not used at this moment
...
#             relevant_centerlines_concat = relevant_centerlines.view(batch_size,-1,2)
#             relevant_centerlines_concat = relevant_centerlines_concat.contiguous().view(batch_size,-1,2)
...
class Multimodal_DecoderLSTM(nn.Module):
    ...
    def forward(self, last_obs, last_obs_rel, state_tuple):
        ...
        _, batch_size, data_dim = last_obs.shape
        last_obs_rel = last_obs_rel.view(1,batch_size,1,data_dim)
        last_obs_rel = last_obs_rel.repeat_interleave(self.num_modes, dim=2)
        last_obs_rel = last_obs_rel.view(1, batch_size, -1)
...
class Centerline_Encoder(nn.Module):
    def __init__(self):
        ...
        # dims = [self.lane_length*self.concat_centerlines, H_DIM, H_DIM]
        dims = [self.lane_length*2, 64, 64]
    ...
    def forward(self, phy_info):
        ...
        # phy_info_ = self.conv1(phy_info_.permute(0,2,1))
        # phy_info_ = self.maxpooling1(phy_info_)
        # phy_info_ = self.conv2(phy_info_)
        # phy_info_ = self.maxpooling2(phy_info_)
        # phy_info_ = self.conv3(phy_info_)
        ...
        phy_info_ = phy_info.view(batch_size, -1) # batch_size x (lane_length · data_dim)
...
class TrajectoryGenerator(nn.Module):
    def __init__(self, PHYSICAL_CONTEXT="dummy"):
        ...
        self.num_modes = ARGOVERSE_NUM_MODES
        self.h_dim = H_DIM
        self.num_sources = 3 # of information in the concatenation
        self.embedding_dim = EMBEDDING_DIM
        ...
        self.physical_context = PHYSICAL_CONTEXT
        global CONCAT_CENTERLINES
        if self.physical_context == "oracle":
            CONCAT_CENTERLINES = 1
        elif self.physical_context == "plausible_centerlines+area":
            CONCAT_CENTERLINES = 6
        ...
        mid_dim = math.ceil((self.h_dim*self.num_sources + self.h_dim)/2)
        dims = [self.h_dim*self.num_sources,mid_dim,self.h_dim]
        self.mlp_decoder_context = make_mlp(dims,
                            activation_function="GELU",
                            batch_norm=False,
                            dropout=DROPOUT)

        out_dim = self.h_dim*2 + 64
        self.speed_decoder = nn.LSTM(input_size=self.embedding_dim, hidden_size=out_dim) 
        # self.decoder = Multimodal_DecoderLSTM(PHYSICAL_CONTEXT=self.physical_context)
        # self.fc_speed    = nn.Linear(in_features=self.h_dim*self.num_sources, out_features=self.data_dim)
        # self.fc_speed    = nn.Linear(in_features=self.h_dim, out_features=self.data_dim)
        pred = []

        # out_dim = self.h_dim*self.num_sources

        for _ in range(self.num_modes):
            pred.append(nn.Linear(out_dim, self.data_dim))
            # pred.append(Linear(self.h_dim, self.data_dim))
        self.fc_speed = nn.ModuleList(pred)

        mid_dim = math.ceil((out_dim + self.data_dim)/2)
        dims = [out_dim,mid_dim,self.data_dim]
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, agent_idx, phy_info=None, relevant_centerlines=[]):
        ...
        # Motion Encoder (Only the target agent)
        ...
        speed_outputs_modes = []
        last_pos = pos[-1,:,:].unsqueeze(0)
        last_speed = speed[-1,:,:].unsqueeze(0)

        # Social interactions (GNN + MHSA)
        ...
        # if torch.any(encoded_social_info.isnan()):
        #     pdb.set_trace()
        # elif np.any(agent_idx): # single agent
        #     encoded_social_info = encoded_social_info[agent_idx,:]
        
        # phy_info = self.add_noise(phy_info)

        # Concat relevant centerlines

        # if self.physical_context == "plausible_area+centerlines":
        #     if self.training: # Get random concatenation
        #         dim = 0 # N centerlines
        #         idx = torch.randperm(relevant_centerlines.shape[dim])
        #         # pdb.set_trace()
        #         relevant_centerlines_random = relevant_centerlines[idx,:,:,:] # Change order of centerlines for the whole batch
        #         relevant_centerlines_concat = relevant_centerlines_random.contiguous().view(batch_size,-1,2)
        #     else:
        #         relevant_centerlines_concat = relevant_centerlines.contiguous().view(batch_size,-1,2)
        #     encoded_phy_info = self.centerline_encoder(relevant_centerlines_concat)
        # elif self.physical_context == "oracle":
        #     encoded_phy_info = self.centerline_encoder(phy_info)

        # encoded_info = torch.cat([hpo.squeeze(0),hsp.squeeze(0), encoded_social_info, encoded_phy_info],dim=1)
        # encoded_info = torch.cat([hpo.squeeze(0),hsp.squeeze(0), encoded_phy_info],dim=1)

        # if torch.any(encoded_info.isnan()):
        #     pdb.set_trace()
        ...
        # state_tuple = (hds,cds)
        # pred_traj_fake_rel, conf = self.decoder(last_pos, last_speed, state_tuple)
        # pdb.set_trace()
        for mode in range(self.num_modes):
            ...
            encoded_phy_info = self.centerline_encoder(relevant_centerlines[mode,:,:,:])
            encoded_info = torch.cat([hpo.squeeze(0), hsp.squeeze(0), encoded_phy_info],dim=1)
            ...
            for t in range(self.pred_len):
                ...
                speed_output = self.fc_speed[mode](hds)
